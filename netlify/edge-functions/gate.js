...
<div class="card">
  ...
  <p>Crew: sign in with your myaccesspartners.com Google account. Partners: open the link you were sent, or enter your access key.</p>
...
(async () => {
  ...
  async function admit(session){
    ...
    if (r.ok) { history.replaceState(null, "", clean()); location.reload(); return; }
  ...
  document.getElementById("g").onclick = async () => {
    err.textContent = "";
    const { error } = await sb.auth.signInWithOAuth({ provider: "google", options: { redirectTo: clean() } });
